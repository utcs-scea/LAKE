...
struct completion batch_completed[3];
struct completion batch_block[3];
struct completion finalize_batch[3];
//DEFINE_SPINLOCK(batch_running_lock);
spinlock_t batch_running_lock[3];
//DEFINE_SPINLOCK(batch_exited);
spinlock_t batch_exited[3];

int is_batch_running[3];
int n_exited[3];
int this_batch_size[3];
int pending[3];
u64 window_start_ns[3];
u64 waiting[3];

//GPU inference variables
struct GPU_weights gpu_weights[3]; //per-ssd weights, we are not going to have more than 3 ssds..
int use_cpu_instead[3];
int is_batch_blocked[3];

void predictors_mgpu_init() {
	int i;
	for (i=0 ; i < 3 ; i++) {
...
void multi_gpu_predict_batch(char *__feat_vec, int n_vecs, long **weights, int dev) {
	//do inference
	void *args[] = {
		&weights[0], &weights[2], &d_input_vec_i[dev], &d_mid_res_i[dev]
	};
	void *args1[] = {
		&weights[1], &weights[3], &d_mid_res_i[dev], &d_final_res_i[dev]
	};
...
bool gpu_batch_entry(char *feat_vec, int n_vecs, long **weights) {
	...
	memcpy((multi_inputs_to_gpu[this_dev_idx]) +(my_id*LEN_INPUT), feat_vec, LEN_INPUT);
	 ...
	 */
		...
		if(my_arrival - window_start_ns >= window_size_ns || waiting[this_dev] == max_batch_size) {

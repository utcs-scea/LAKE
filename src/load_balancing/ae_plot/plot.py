...
for line in reversed(klog.splitlines()):
    print (line)
    if "MLLB_GPU_batch_" in line:
        #format:  GPU_batch_X,2,4     (2 is comp, 4 is comp+data)
        m = re.search("MLLB_GPU\_batch\_(\d+), (\d+), (\d+)", line) 
        if not m:
    ...
    if "MLLB_CPU_batch_" in line:
        #format:  CPU_batch_X,2
        m = re.search("MLLB_CPU\_batch\_(\d+), (\d+), (\d+)", line) 
        if not m:
...
fig.savefig(f"mllb_ae.pdf", bbox_inches='tight', pad_inches=0.05)

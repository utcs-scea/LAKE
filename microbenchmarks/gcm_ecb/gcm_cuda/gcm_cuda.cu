...
void AES_GCM_encrypt(uint8_t* dst, uint8_t* src, uint32_t size) {
    ...
    AES_GCM_xcrypt(dst, nonce_device, src, size);
    AES_GCM_compute_mac(&dst[size], nonce_device, dst, size);
}
...
void AES_GCM_decrypt(uint8_t* dst, uint8_t* src, uint32_t size) {
    assert(size % AES_BLOCKLEN == 0);
    AES_GCM_compute_mac(dst, nonce_device, src, size);
...
//     AES_GCM_encrypt((uint8_t*)d_cipher, (uint8_t*)d_plain, plainlen);
//     gpuErrchk(cudaDeviceSynchronize());

//     AES_GCM_decrypt((uint8_t*)d_decrypted, (uint8_t*)d_cipher, plainlen);
//     gpuErrchk(cudaDeviceSynchronize());

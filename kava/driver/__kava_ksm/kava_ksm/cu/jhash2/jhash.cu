 ...
 * @shift: bits to roll
 */
__device__ static inline uint32_t rol32(uint32_t word, unsigned int shift)
{
 ...
 */
__device__ static inline uint32_t jhash2(const uint32_t *k, uint32_t length, uint32_t initval)
{
	uint32_t a, b, c;
...
__global__ void jhash2(void *input, uint32_t *output) {
  size_t len = 4096 / sizeof(uint32_t); 
  uint32_t seed = 17;
  int idx =
...
#else
  *out_addr = jhash2((uint32_t *) in_addr, len, seed);
#endif
}

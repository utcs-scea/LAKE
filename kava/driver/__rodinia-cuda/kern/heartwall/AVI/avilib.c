...
static size_t avi_read(struct file *fd, char *buf, size_t len, loff_t *pos)
{
   size_t n = 0;
   size_t r = 0;

   while (r < len) {
      n = kernel_read (fd, buf + r, len - r, pos);
...
static size_t avi_write (struct file *fd, char *buf, size_t len, loff_t *pos)
{
   size_t n = 0;
   size_t r = 0;

   while (r < len) {
      n = kernel_write(fd, (const void *)buf + r, len - r, pos);
      if (n < 0)
         return n;
...
static int avi_add_chunk(avi_t *AVI, unsigned char *tag, unsigned char *data, int length)
{
   ...
   if( avi_write(AVI->fdes,(char *)c,8, &AVI->kpos) != 8 ||
       avi_write(AVI->fdes,(char *)data,length, &AVI->kpos) != length )
   {
       AVI->kpos = AVI->pos;
...
static int avi_add_index_entry(avi_t *AVI, unsigned char *tag, long flags, unsigned long pos, unsigned long len)
{
   ...
   if(AVI->n_idx>=AVI->max_idx) {
       ptr = (void *)vmalloc((AVI->max_idx+4096)*16);
       memcpy(ptr, (void *)AVI->idx, AVI->max_idx * 16);
       vfree((void *)AVI->idx);
...
avi_t* AVI_open_output_file(char * filename)
{
   ...
   unsigned char *AVI_header = vmalloc(sizeof(unsigned char) * HEADERBYTES);

   /* Allocate the avi_t struct and zero it */

   AVI = (avi_t *) vmalloc(sizeof(avi_t));
   if(AVI==0)
   {
      AVI_errno = AVI_ERR_NO_MEM;
      return 0;
  ...
  /* mask = umask (0);
   ...
   AVI->fdes = filp_open(filename, O_RDWR|O_CREAT|O_BINARY, 0644 &~ mask);
   if (AVI->fdes == NULL)
   {
   ...
   i = avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES, &AVI->kpos);
   if (i != HEADERBYTES)
   {
      filp_close(AVI->fdes, NULL);
      AVI_errno = AVI_ERR_WRITE;
      vfree(AVI);
      return 0;
   }
...
void AVI_set_video(avi_t *AVI, int width, int height, long fps, char *compressor)
{
...
int AVI_set_audio(avi_t *AVI, int channels, long rate, int bits, int format, long mp3rate)
{
   ...
   if(AVI->anum > AVI_MAX_TRACKS) {
     pr_err("error - only %d audio tracks supported\n", AVI_MAX_TRACKS);
     return 1;
   }
...
int avi_update_header(avi_t *AVI)
{
   ...
   if(AVI->fps < 10) {
     ...
     frate = (int) (FRAME_RATE_SCALE*AVI->fps + 5000) / 10000;
     ms_per_frame=(int) (1000000L*10000*10/AVI->fps + 5)/10;
   }
...
#define AVIF_COPYRIGHTED        0x00020000
   ...
   if(njunk<=0)
     {
       pr_err("AVI_close_output_file: # of header bytes too small\n");
       return 1;
     }
   ...
   AVI->kpos = 0;
   if (avi_write(AVI->fdes,(char *)AVI_header,HEADERBYTES, &AVI->kpos)!=HEADERBYTES) {
...
int AVI_append_audio(avi_t *AVI, char *data, long bytes)
{
  ...
  avi_write(AVI->fdes,(char *) c, 4, &AVI->kpos);

  AVI->kpos = pos+8+length;

  i=PAD_EVEN(length + bytes);

  bytes = i - length;
  avi_write(AVI->fdes, data, bytes, &AVI->kpos);
  AVI->pos = pos + 8 + i;
...
int AVI_close(avi_t *AVI)
{
   ...
   filp_close(AVI->fdes, NULL);
   if(AVI->idx) vfree(AVI->idx);
   if(AVI->video_index) vfree(AVI->video_index);
   //FIXME
   //if(AVI->audio_index) vfree(AVI->audio_index);
   vfree(AVI);
...
avi_t *AVI_open_input_file(char *filename, int getIndex)
{
  ...
  AVI->fdes = filp_open(filename, O_RDONLY|O_BINARY, 0);
  if(AVI->fdes < 0)
    {
      AVI_errno = AVI_ERR_OPEN;
      vfree(AVI);
...
avi_t *AVI_open_fd(struct file *fd, int getIndex)
{
  avi_t *AVI=NULL;
...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  /* Read first 12 bytes and check that this is an AVI file */
   ...
   if( avi_read(AVI->fdes,data,12, &AVI->kpos) != 12 ) ERR_EXIT(AVI_ERR_READ)
   ...
   while(1)
   {
      if( avi_read(AVI->fdes,data,8, &AVI->kpos) != 8 ) break; /* We assume it's EOF */
      ...
      {
         if( avi_read(AVI->fdes,data,4, &AVI->kpos) != 4 ) ERR_EXIT(AVI_ERR_READ)
         n -= 4;
         if(strncasecmp(data,"hdrl",4) == 0)
         {
            hdrl_len = n;
            hdrl_data = (unsigned char *) vmalloc(n);
            if(hdrl_data==0) ERR_EXIT(AVI_ERR_NO_MEM);

	    // offset of header

	    header_offset = AVI->kpos;

         if( avi_read(AVI->fdes,(char *)hdrl_data,n, &AVI->kpos) != n ) ERR_EXIT(AVI_ERR_READ)
         }
         else if(strncasecmp(data,"movi",4) == 0)
         {
            AVI->movi_start = AVI->kpos;
            AVI->kpos += n;
         }
         else
             AVI->kpos += n;
      ...
      else if(strncasecmp(data,"idx1",4) == 0)
      {
         ...
         AVI->idx = (unsigned  char((*)[16]) ) vmalloc(n);
         if(AVI->idx==0) ERR_EXIT(AVI_ERR_NO_MEM)
         if(avi_read(AVI->fdes, (char *) AVI->idx, n, &AVI->kpos) != n) ERR_EXIT(AVI_ERR_READ)
      }
      else
          AVI->kpos += n;
   }
   ...
   for(i=0;i<hdrl_len;)
   {
      ...
      if(strncasecmp((char *)hdrl_data+i,"strh",4)==0)
      {
	    ...
	    AVI->v_codech_off = header_offset + i+4;
            ...
            if(scale!=0) AVI->fps = 10000*rate/scale;
   ...
   if(AVI->idx)
   {
      ...
      AVI->kpos = pos;
      if(avi_read(AVI->fdes,data,8, &AVI->kpos)!=8) ERR_EXIT(AVI_ERR_READ)
   ...
   if(idx_type == 0)
   {
      ...
      while(1)
      {
         if( avi_read(AVI->fdes,data,8, &AVI->kpos) != 8 ) break;
         n = str2ulong((unsigned char *)data+4);
	     ...
	     || ( (data[2]=='w' || data[2]=='W') &&
		  (data[3]=='b' || data[3]=='B') ) )
	   {
    	   avi_add_index_entry(AVI,(unsigned char *) data,0,AVI->kpos-8,n);
         }
...
//   pr_err("chunks = %ld %d %s\n", AVI->track[0].audio_chunks, AVI->anum, AVI->track[0].audio_tag);

   if(AVI->video_frames==0) ERR_EXIT(AVI_ERR_NO_VIDS);
   AVI->video_index = (video_index_entry *) vmalloc(nvi*sizeof(video_index_entry));
   if(AVI->video_index==0) ERR_EXIT(AVI_ERR_NO_MEM);
   
   for(j=0; j<AVI->anum; ++j) {
       if(AVI->track[j].audio_chunks) {
	   AVI->track[j].audio_index = (audio_index_entry *) vmalloc(nai[j]*sizeof(audio_index_entry));
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, int *keyframe)
{
   ...
   *keyframe = (AVI->video_index[AVI->video_pos].key==0x10) ? 1:0;
   AVI->kpos = AVI->video_index[AVI->video_pos].pos;
   if (avi_read(AVI->fdes,vidbuf,n, &AVI->kpos) != n)
   {
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
   ...
   while(bytes>0)
   {
      ...
      AVI->kpos = pos;
      if (avi_read(AVI->fdes,audbuf+nr,todo, &AVI->kpos) != todo)
      {
         AVI_errno = AVI_ERR_READ;
         return -1;
      }
 ...
 */
   ...
   while(1)
   {
      ...
      if( avi_read(AVI->fdes,data,8, &AVI->kpos) != 8 ) return 0;
      ...
      if(strncasecmp(data,AVI->video_tag,3) == 0)
      {
         *len = n;
         ...
         if(avi_read(AVI->fdes,vidbuf,n, &AVI->kpos) != n ) return 0;
         return 1;
      }
      else if(strncasecmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
      {
         *len = n;
         ...
         if(avi_read(AVI->fdes,audbuf,n, &AVI->kpos) != n ) return 0;
         return 2;
         break;
      }
      else

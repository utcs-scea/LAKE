...
int* chop_flip_image(char *image, 
	...
	height_new = bottom - top + 1;
	width_new = right - left + 1;

	// allocate memory for cropped/flipped frame
	result = (int *) vmalloc(height_new * width_new * sizeof(int));
	...
	if (scaled) {
		...
		for(i = 0; i <height_new; i++){				// rows
			for(j = 0; j <width_new; j++){			// colums
				temp = image[((height - 1 - (i + top)) * width) + (j + left)] / scale;
				if(temp<0){
...
// convert storage method (from row-major to column-major)
    result_converted = (int *) vmalloc(height_new * width_new * sizeof(int));
	if(converted==1){
...
int* get_frame(	avi_t* cell_file, 
	...
	image_buf = (char*) vmalloc(width * height * sizeof(char));

...
(CUmodule mod, int gdx, int gdy, int bdx, int bdy, int _lambda, int Nr, int Nc, 
 ...
 CUdeviceptr d_dW, int q0sqr, CUdeviceptr d_c, CUdeviceptr d_I)
{
	CUfunction f;
	CUresult res;
...
int main(int argc, char *argv [])
{
	...
	image_ori = (int*) malloc(sizeof(int) * image_ori_elem);
	...
	image = (int*) malloc(sizeof(int) * Ne);

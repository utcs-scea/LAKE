...
{
    /* Create command ring */
    handle->buf = vmalloc(MMAP_NUM_PAGES << PAGE_SHIFT);
...
static ssize_t chardev_read(struct file *filp, char __user *buf, size_t size, loff_t *offp)
{
    ...
    copy_to_user(buf, handle->buf, size);
    spin_unlock(&handle->lock);

    return size;
}

static ssize_t chardev_write(struct file *filp, const char __user *buf, size_t size, loff_t *offp)
{
    spin_lock(&handle->lock);
    copy_from_user(handle->buf, buf, size);
    spin_unlock(&handle->lock);

...
#define KAVA_SET_MESSAGE_SIZE _IOR(UPCALL_TEST_DEV_MAJOR_NUM, 0x3, int)
#define KAVA_SEND_MESSAGE _IOR(UPCALL_TEST_DEV_MAJOR_NUM, 0x4, uintptr_t)
#define KAVA_RECV_MESSAGE _IOW(UPCALL_TEST_DEV_MAJOR_NUM, 0x5, uintptr_t)

/**
 * Shared memory
 */
#define MMAP_NUM_PAGES 1024

struct shared_region {
    volatile int doorbell_kern;
    volatile int doorbell_user;
    size_t size;
    char buffer[0];
};
                            ...
                            unsigned long arg)
{
    ...
    switch (cmd) {
        case KAVA_TEST_START_UPCALL:
            ...
            for (i = 0; i < arg; i++) {
                getnstimeofday(&ts);
                pr_info("Upcall[%lu] called: sec=%lu, usec=%lu\n", i, ts.tv_sec, ts.tv_nsec / 1000);
